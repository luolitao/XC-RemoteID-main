...
void MockData::init(RIDData &data)
{
    ...
    strncpy(data.uas_id,   "ESP32MOCKSIMBD1234AB", 20);
    ...
    _last_ms = millis();
    Serial.println("[XC-RID] RIDDATA initialized");
}
...
void MockData::update(RIDData &data)
{
    ...
    // Serial.println("[XC-RID] Mock data updated - Lat: " + String(data.lat, 6) + ", Lon: " + String(data.lon, 6) + ", Alt: " + String(data.geo_alt_m) + "m");
}
